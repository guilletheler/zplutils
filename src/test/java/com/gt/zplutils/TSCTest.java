...
public class TSCTest {
	...
	public static boolean sendUsbControlIrp(UsbDevice usbDevice, UsbControlIrp usbControlIrp) {
		try {
			/*
			 * This will block until the submission is complete. Note that submissions
			 * (except interrupt and bulk in-direction) will not block indefinitely, they
			 * will complete or fail within a finite amount of time. See
			 * MouseDriver.HidMouseRunnable for more details.
			 */
			usbDevice.syncSubmit(usbControlIrp);
			return true;
		} catch (UsbException uE) {
			/*
			 * The exception sould indicate the reason for the failure. For this example,
	...
	public void printZpl() throws ZebraPrintException, UnsupportedEncodingException {
		...
		try {
			...
			if (psZebra == null) {
				assertTrue("No hay impresora para probar", true);
				return;
//                throw new ZebraPrintNotFoundException("Zebra printer not found : " + printerName);
	...
	@Test
	public void print() throws ZebraPrintException, InterruptedException {
		// ZebraBarCode128 barcode = new ZebraBarCode128(70, 1000,
		// "0235600703875191516022937128", 190, false, 4, 2);
		try {
			String tspl = "SIZE 57 mm,37 mm \n" +
					"GAP 3 mm,0 \n" +
					"DIRECTION 0 \n" +
			...
			try {
				...
				endpoints.forEach((p) -> {
					System.out.println("Direction: " + p.getDirection() + ", Address: "
							+ p.getUsbEndpointDescriptor().bEndpointAddress());
				});
				...
				try {
					...
					writePipe.syncSubmit("QRCODE 10,10,L,4,A,0,M2,S7,\"http://fintrace.tk?sc=9krPWOrxzsL\"\n"
							.getBytes(Charset.forName("US-ASCII")));
					// writePipe.syncSubmit("TEXT 10,10,\"3\",0,1,1, \"Encoded data:
					// \"".getBytes(Charset.forName("US-ASCII")));
					// writePipe.syncSubmit(new byte[] {0x0D});
...
//                }
			...
			System.out.println("Error de puerto usb");
